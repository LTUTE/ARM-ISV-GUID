...
```
4.	По умолчанию, мы подключаемся в подписку по умолчанию (default subscription). При необходимости, переключаемся на подписку содержащую интересующие нас ресурсы: 
```PowerShell
